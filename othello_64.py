...
def min_spaces():    # 相手が石を置けるマスが少なくなるように手を選択
    save_board()
    win = []
    for i in range(8):
        win.append([0] * 8)
        ...
        for x in range(8):
            if reverse_stones_count(x, y, BLACK) > 0:   # 石が置ける場合
                ...
                for i in range(loops):
                    ...
                    white_stone, black_stone = stones_count()
...
def game_progress():
    ...
    if progress == 0:   # ゲーム開始前
        ...
        if turn == 1:
            ...
            while True:   # ランダム
                ...
                if reverse_stones_count(mx, my, turn) > 0:   # 石を置ける場合
                    ...
                    break
            '''''
            # cx, cy = min_spaces()    # その後に相手が石を置けるマスの数を最小にする
            cx, cy = montecarlo(10)   # モンテカルロ法
        ...
        progress = 0
    root.after(100, game_progress)   # 100ミリ秒後にgame_progress関数を呼び起こす
    return
